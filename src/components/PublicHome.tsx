...
export const PublicHome: React.FC<PublicHomeProps> = ({
  ...
  // Make an Appointment Form States
  ...
  const [selectedLocation, setSelectedLocation] = useState('Vadapalani');
  ...
  // Dynamically populated specialties from doctors master data
  const availableSpecialties = useMemo(() => {
    const depts = doctors.map((d) => d.department).filter(Boolean);
  ...
  useMemo(() => {
    ...
    const [y, m, d] = patientDob.split('-').map(Number);
    let ageCalc = today.getFullYear() - y;
    const monthDiff = today.getMonth() - (m - 1);
    if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < d)) {
      ageCalc--;
    }
    setPatientAge(ageCalc >= 0 ? String(ageCalc) : '');
  }, [patientDob]);
  ...
  // Make an Appointment Form Submit Handler
  const handleAppointmentSubmit = async (e: React.FormEvent) => {
    ...
    if (!patientAge || Number.isNaN(ageNum) || ageNum <= 0 || ageNum > 130) {
    ...
    if (!selectedLocation) {
      setFormError('Please select a hospital location.');
      return;
    }
    ...
    if (!symptoms.trim()) {
      setFormError('Please describe your symptoms or reason for consultation.');
      return;
    }
    ...
    if (onBookAppointmentDirect) {
      const res = await onBookAppointmentDirect({
        ...
        patient_phone: `+91 ${mobileNumber.trim()}`,
        ...
        fee_amount: 700,
        is_opd_registration: true
      });

      setIsSubmittingForm(false);

      if (res.success) {
        const uhidGen = (res as any).appointment?.patient_uhid || `UHID-2026-${String(Math.floor(1000 + Math.random() * 9000))}`;
        setAppointmentSuccessModal({
          token: res.appointment_no || (res as any).appointment?.appointment_no || 'APT-2026-OPD',
          uhid: uhidGen,
          patientName: fullName,
          location: selectedLocation,
          specialty: selectedSpecialty,
          doctor: 'To be assigned by Receptionist',
        ...
        setFormError(res.error || 'Could not schedule appointment.');
        if (res.error && res.error.toLowerCase().includes('already exists')) {
          window.alert('User already exists for this phone number');
        }
      }
    } else {
      setIsSubmittingForm(false);
  ...
  return (
    <div className="w-full bg-slate-50 text-slate-900 font-sans">
      ...
      {/* =========================================================================
          SECTION 1: MAKE AN APPOINTMENT FORM (Centered, Clean Healthcare Layout)
          ========================================================================= */}
      <motion.section
        id="book-appointment"
        ...
        className="bg-gradient-to-b from-[#002a54] via-[#003876] to-[#002244] text-white py-12 px-4 sm:px-6 relative shadow-inner"
      >
        <div className="max-w-4xl mx-auto">
          {/* Centered Heading & Subtitle */}
          <div className="text-center mb-8">
            <span className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#f58220]/20 text-amber-300 text-xs font-black uppercase tracking-widest border border-[#f58220]/40 mb-3 shadow-xs">
            ...
            <p className="text-xs sm:text-sm text-slate-200 mt-2 max-w-xl mx-auto leading-relaxed">
              Schedule your consultation with our specialist doctors across Chennai &amp; Tamil Nadu. Registration is completed first without upfront fee collection.
            </p>
          </div>

          {/* Clean Professional Form Card */}
          <div className="bg-white text-slate-900 rounded-3xl shadow-2xl border border-slate-200 p-6 sm:p-8 lg:p-10">
            {formError && (
            ...
            <form onSubmit={handleAppointmentSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium text-slate-900 placeholder-slate-400 outline-none focus:bg-white focus:ring-2 focus:ring-[#003876] transition shadow-2xs"
                    required
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {patientAge && (
                    <p className="text-[11px] text-slate-500 font-medium mt-1 flex items-center gap-1">
                      ...
                      Age: <span className="font-bold text-slate-700">{patientAge} years</span>
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs font-medium text-slate-900 outline-none focus:bg-white focus:ring-2 focus:ring-[#003876] transition shadow-2xs cursor-pointer"
                    required
                  >
              ...
              <div>
                ...
                <div className={`flex items-center rounded-xl overflow-hidden border transition shadow-2xs ${
                  mobileError ? 'border-rose-500 bg-rose-50/20' : 'border-slate-300 bg-slate-50 focus-within:bg-white focus-within:ring-2 focus-within:ring-[#003876]'
                }`}>
                  <div className="flex items-center gap-1.5 px-3.5 py-2.5 bg-slate-100 border-r border-slate-300 text-xs font-bold text-slate-700 select-none shrink-0">
                    <span>🇮🇳</span>
                    <span>+91</span>
                  </div>
                  <input
                    ...
                    className="w-full px-3 py-2.5 text-xs text-slate-900 font-medium placeholder-slate-400 outline-none bg-transparent"
                    required
                  />
                </div>
                {mobileError && (
              ...
              {/* Row 4: Select Location & Select Specialty */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1.5">
                    Select Location <span className="text-rose-600">*</span>
                  </label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1.5">
                    Select Specialty <span className="text-rose-600">*</span>
                  </label>
                  <select
              ...
              {/* Row 5: Symptoms / Reason for Consultation */}
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1.5">
                  Symptoms / Reason for Consultation <span className="text-rose-600">*</span>
                </label>
                <textarea
                  rows={3}
                  placeholder="Please describe your symptoms, complaints, or health issues (e.g., chest tightness, fever, joint pain, routine checkup)..."
                  value={symptoms}
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={isSubmittingForm || Boolean(mobileError && mobileNumber.length > 0)}
                  className="w-full py-3.5 px-6 bg-[#f58220] hover:bg-[#e07113] text-white font-extrabold text-sm sm:text-base rounded-2xl shadow-lg transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.01]"
                >
                  <i className="fa-regular fa-calendar-check text-lg"></i>
                  <span>{isSubmittingForm ? 'Submitting Appointment...' : 'Submit Appointment'}</span>
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 sm:p-7 shadow-2xl border border-slate-200 relative overflow-hidden">
            ...
            <h3 className="text-xl sm:text-2xl font-black text-center text-slate-900 mb-1 font-display">
              Make an Appointment Confirmed
            </h3>
            <p className="text-xs text-center text-slate-500 mb-4 font-medium">
              Your appointment has been successfully recorded in the hospital management database.
            </p>
            
            <div className="bg-slate-50 p-4 rounded-2xl space-y-2.5 text-xs border border-slate-200/80 mb-4">
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Patient ID (UHID):</span>
                <span className="font-mono font-black text-emerald-800 bg-emerald-50 px-2.5 py-0.5 rounded-lg border border-emerald-200">
                  {appointmentSuccessModal.uhid}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Appointment Token:</span>
                <span className="font-mono font-black text-[#e66c00] bg-amber-50 px-2.5 py-0.5 rounded-lg border border-amber-200">
                  {appointmentSuccessModal.token}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Patient Name:</span>
                <span className="font-bold text-slate-900">{appointmentSuccessModal.patientName}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Location:</span>
                <span className="font-bold text-slate-900">{appointmentSuccessModal.location}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Specialty / Department:</span>
                <span className="text-slate-700 font-medium">{appointmentSuccessModal.specialty}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Attending Specialist:</span>
                <span className="font-bold text-slate-900">{appointmentSuccessModal.doctor}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-bold">Contact Mobile:</span>
                <span className="font-bold text-slate-900 font-mono">+91 {appointmentSuccessModal.phone}</span>
              </div>
              <div className="border-t border-slate-200 pt-2 text-[11px] text-slate-600 space-y-1">
                <div className="font-bold text-slate-800 flex items-center gap-1">
                  <i className="fa-solid fa-stethoscope text-sky-600"></i> Next Step:
                </div>
                <p>
                  Please visit the hospital OPD desk. You will proceed to doctor consultation first. OPD fee of <strong>₹900 (₹700 consultation + ₹200 Clinical Nursing)</strong> + applicable tax will be collected after consultation.
                </p>
              </div>
            </div>

            <button
              onClick={() => setAppointmentSuccessModal(null)}
                    ...
                    <div className="p-5 pt-0 border-t border-slate-100 flex flex-col gap-2.5 mt-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <a
                          href="#book-appointment"
                          onClick={() => {
                            setSelectedSpecialty(doc.department);
                          }}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-2">
              ...
              <a
                href="#book-appointment"
                onClick={() => {
                  setSelectedSpecialty(activeDoctorModal.department);
                  setActiveDoctorModal(null);
                }}
                className="flex-1 py-2.5 bg-[#f58220] hover:bg-[#e07113] text-white font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition cursor-pointer"
              >
