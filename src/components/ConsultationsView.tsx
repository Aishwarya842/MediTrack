...
interface ConsultationsViewProps {
  ...
  onWritePrescription?: (target: { patientId: number; doctorId: number; diagnosis: string; symptoms?: string; vitals?: { bp?: string; pulse?: number | string; temperature?: string; spo2?: string }; notes?: string }) => void;
}
...
export const ConsultationsView: React.FC<ConsultationsViewProps> = ({
  ...
  // Filter consultations strictly by Doctor / Department / Patient
  const filteredConsultations = consultations.filter((c) => {
    if (isDoctor) {
      const docId = currentUser?.doctor_id;
      const cleanDocName = doctorName?.trim().toLowerCase();
      const matchDoc =
        (docId && c.doctor_id === docId) ||
        (cleanDocName && c.doctor_name && c.doctor_name.trim().toLowerCase() === cleanDocName);
      if (!matchDoc) return false;
    } else if (isPatient) {
      const matchPat =
        (currentUser?.patient_uhid && c.patient_uhid?.trim().toUpperCase() === currentUser.patient_uhid.trim().toUpperCase()) ||
        (currentUser?.patient_id && c.patient_id === currentUser.patient_id);
  ...
  // Auto-fill Patient Clinical Vitals from the receptionist-recorded values on the patient's Health Card
  useEffect(() => {
    if (!isModalOpen || typeof patientId !== 'number') return;
    ...
    const bpSource =
      p.bp
        ...
        : p.bp_systolic && p.bp_diastolic
          ...
          : '';
    setBp(bpSource);
    setPulse(p.pulse ? String(p.pulse) : '');
    setTemperature(p.temperature ? p.temperature.replace('°F', '') : '');
    setSpo2(p.spo2 || '');
  }, [isModalOpen, patientId, patients]);
  ...
  const handleSaveConsultation = async (e: React.FormEvent, proceedToRx: boolean = false) => {
    ...
    await onAddConsultation({
      ...
      temperature: temperature.trim() ? `${temperature.trim()}°F` : '',
      spo2,
  ...
  const handleOpenEdit = (con: Consultation) => {
    ...
    setEditBp(con.bp?.replace(' mmHg', '') || '');
    setEditPulse(con.pulse || '');
    setEditTemperature(con.temperature?.replace('°F', '') || '');
    setEditSpo2(con.spo2 || '');
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    const updated: Consultation = {
      ...
      bp: editBp.trim() ? `${editBp.trim()} mmHg` : '',
      pulse: Number(editPulse) || 0,
      temperature: editTemperature.trim() ? `${editTemperature.trim()}°F` : '',
      spo2: editSpo2,
    ...
    if (onWritePrescription) {
      onWritePrescription({
        ...
        vitals: {
          bp: updated.bp || '',
          pulse: updated.pulse || 0,
          temperature: updated.temperature || '',
          spo2: updated.spo2 || ''
        },
        notes: updated.clinical_notes
      });
    } else {
      alert(`Consultation note updated successfully for ${updated.patient_name}!`);
    }
  };
        ...
        <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-3 px-4 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-emerald-950">
              Personal EHR Consultations: Showing clinical examination records exclusively for {currentUser?.full_name} ({currentUser?.patient_uhid}).
            </span>
          </div>
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs">
        <div>
          <h2 className="text-lg font-bold text-slate-900">
            {isPatient ? 'My Clinical Consultation Records & Doctor Diagnosis' : 'Doctor OPD Clinical Consultation Workspace'}
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            {isPatient
              ? 'Verified clinical examinations, diagnoses, vital observations, and doctor assessments.'
                    ...
                    <td className="p-3 text-right whitespace-nowrap">
                      {!isPatient ? (
                        ...
                        <button
                          ...
                          className="bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 px-2.5 py-1 rounded-lg text-[11px] font-semibold transition cursor-pointer inline-flex items-center gap-1 shadow-2xs"
                        >
                          <i className="fa-solid fa-file-prescription text-emerald-600"></i> View Prescriptions
                        </button>
                      )}
                    </td>
                  </tr>
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      placeholder={pulse ? '' : 'Not recorded'}
                      value={pulse}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder={temperature ? '' : 'Not recorded'}
                      value={temperature}
