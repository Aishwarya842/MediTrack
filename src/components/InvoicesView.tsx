...
import { Invoice, Patient, Doctor, Appointment } from '../data/hospitalData';
...
export const InvoicesView: React.FC<InvoicesViewProps> = ({
  ...
  // Strict Patient Isolation for invoices
  const displayedInvoices = React.useMemo(() => {
    return invoices.filter((inv) => {
      if (isPatient) {
        return (
          (currentUser?.patient_uhid &&
            inv.patient_uhid?.trim().toUpperCase() === currentUser.patient_uhid.trim().toUpperCase()) ||
          (currentUser?.patient_id && inv.patient_id === currentUser.patient_id)
        );
      }
      return true;
    });
  }, [invoices, isPatient, currentUser]);

  const [selectedInvoice, setSelectedInvoice] = useState<Invoice | null>(displayedInvoices[0] || null);
  ...
  const [additionalCharges, setAdditionalCharges] = useState(200.0);
  ...
  React.useEffect(() => {
    ...
    } else if (!selectedInvoice || !displayedInvoices.find((inv) => inv.id === selectedInvoice.id)) {
  ...
  const handleGenerateInvoice = async (e: React.FormEvent) => {
    ...
    const matchingAppt = appointments?.find(
      (a) => a.patient_uhid.toUpperCase() === (patUhid || '').toUpperCase()
    );
    const consultationTime =
      matchingAppt?.appointment_time ||
  ...
  const totalAmountPaid = displayedInvoices.reduce((sum, inv) => sum + (inv.total_amount || 0), 0);
    ...
    <div className="space-y-4">
        ...
        <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-3 px-4 flex flex-wrap items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-emerald-950">
              Personal Financial Statement: Showing official GST billing statements issued exclusively for {currentUser?.full_name} ({currentUser?.patient_uhid}).
            </span>
          </div>
          <span className="text-[11px] font-mono font-bold text-emerald-900 bg-white px-2.5 py-0.5 rounded-md border border-emerald-200 shadow-xs">
            {displayedInvoices.length} Invoices Verified
          </span>
        </div>
      )}
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 bg-white p-5 rounded-xl border border-slate-200/80 shadow-xs">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-bold text-slate-900">
              {isPatient ? 'My Invoices & Payment Receipts' : 'Hospital Billing & GST Tax Invoices'}
            </h2>
            {isReceptionist ? (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-slate-50 border border-slate-200/80 px-3.5 py-1.5 rounded-lg text-right print:hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-4 bg-white rounded-xl border border-slate-200/80 shadow-xs p-4 space-y-3">
          <div className="font-semibold text-xs text-slate-700 uppercase tracking-wider border-b border-slate-100 pb-2.5 flex justify-between">
            ...
            <span className="text-slate-400 font-normal">{displayedInvoices.length} Records</span>
          </div>
          ...
          {displayedInvoices.length === 0 ? (
            <div className="py-10 px-4 text-center">
              ...
              <h3 className="text-xs font-semibold text-slate-800 mb-1">0 Invoices Found</h3>
              <p className="text-[11px] text-slate-500 mb-4">
                {isPatient
                  ? `No billing invoices on record for ${currentUser?.full_name} (${currentUser?.patient_uhid}) yet. Your bills will be listed here after receptionist generation.`
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {onDeleteInvoice && isReceptionist && (
                          <button
              ...
              <div className="flex flex-wrap justify-between items-center gap-2 mb-6 pb-4 border-b border-slate-100 print:hidden">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium px-3.5 py-1.5 rounded-lg flex items-center gap-1.5 shadow-xs transition cursor-pointer"
                    title="Print single-sheet NABH Consultation Fee Receipt & Payment Slip"
                  >
                    <i className="fa-solid fa-receipt text-emerald-200"></i> Print Consultation Receipt
                  </button>
                  <button
                    onClick={handlePrint}
            ...
            <form onSubmit={handleGenerateInvoice} className="p-5 overflow-y-auto space-y-3.5 text-xs">
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3.5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-[11px] font-medium text-slate-600 mb-1">Clinical Nursing, Registration &amp; Sanitization (₹)</label>
                    <input
                      type="number"
      ...
      {isReceiptModalOpen && selectedInvoice && (() => {
        const appt = appointments?.find(
          (a) => a.id === selectedInvoice.appointment_id || (selectedInvoice.patient_uhid && a.patient_uhid === selectedInvoice.patient_uhid)
        );
        const pat = patients.find(
          (p) => p.uhid === selectedInvoice.patient_uhid || p.id === selectedInvoice.patient_id || p.full_name === selectedInvoice.patient_name
        );
        const doc = doctors.find((d) => d.id === selectedInvoice.doctor_id || d.full_name === selectedInvoice.doctor_name);
